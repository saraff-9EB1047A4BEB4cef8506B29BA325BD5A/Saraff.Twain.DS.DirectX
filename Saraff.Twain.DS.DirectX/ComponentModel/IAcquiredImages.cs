...
namespace Saraff.Twain.DS.DirectX.ComponentModel {
    ...
    public interface IAcquiredImages {

        Guid Add(Bitmap image);

        void Remove(Guid guid);

        ImageTagInfo GetTagInfo(Guid guid);

        void SetTagInfo(Guid guid, ImageTagInfo tag);

        IEnumerable<Bitmap> Get();

        Bitmap Get(Guid guid);
